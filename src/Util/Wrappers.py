...
def inject_args_ignoring(ignores=None):
...
def inject_args_with(with_=None):
    """DECORATOR: Injects local args into class so that there isn't a lot of:
        def foo(self, x, y, b, w, h, r):
            self.x = x
            self.y = y
            self.b = b
            self.w = w
        NOTE: only includes any names in "with_"
    """
        
    if not hasattr(with_, '__iter__'):
        if isinstance(with_, basestring):
            with_ = [with_]
        else:
            with_ = []
    else:
